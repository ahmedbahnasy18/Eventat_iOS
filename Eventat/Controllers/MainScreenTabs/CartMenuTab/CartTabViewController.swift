...
class CartTabViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.orderLabel.text = NSLocalizedString("order", comment: "")
           self.navigationItem.title = NSLocalizedString("cart", comment: "")
    ...
    @IBAction func bookingBtnAction(_ sender: Any) {
        performSegue(withIdentifier: "goToBookingListFromTab", sender: self)
    }
    @IBAction func orderBtnAction(_ sender: Any) {
        performSegue(withIdentifier: "goToOrderListFromTab", sender: self)
    }
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        
        if(segue.identifier == "goToBookingListFromTab"){
